...
const transform: AxiosTransform = {
  ...
  transformResponseHook: (res: AxiosResponse<Result>, options: RequestOptions) => {
    ...
    if (responseType === ResponseEnum.BLOB) {
      ...
      const blob = content instanceof Blob
        ? content
        : new Blob([JSON.stringify(content) ?? ''], { type: res.headers['content-type'] });
    ...
    throw new Error(msg || '网络请求错误，请稍后重试！');
  },
  ...
  beforeRequestHook: (config, options) => {
    ...
    if (apiUrl && isString(apiUrl)) {
      config.url = `${apiUrl}${
        globSetting.cloudEnable ? (options.region ? '/' + options.region : '') : ''
      }${config.url}`;
    }
  ...
  responseInterceptorsCatch: (_axiosInstance: AxiosInstance, error: any) => {
    ...
    const handleErrorData = (value: unknown) => {
      if (errorMessageMode === 'none') {
        return;
      }

      const normalized = normalizeHttpError(value, response?.status);
      checkStatus(normalized.code, normalized.message, errorMessageMode);
    };
    ...
    if (config?.requestOptions?.responseType === ResponseEnum.BLOB) {
      ...
      reader.addEventListener('loadend', function () {
        //
        try {
          const res = typeof reader.result === 'string' ? JSON.parse(reader.result) : {};
          handleErrorData(res);
        } catch (e) {
          handleErrorData({});
        }
