...
import type { A4ExcelPortraitPrintBo } from '@/api/model/a4ExcelPortraitPrintBo';

interface LodopClient {
  VERSION: string;
  CVERSION: string;
...
declare function getCLodop(): LodopClient | undefined;

let CreatedOKLodopObject: LodopClient | undefined;
let CLodopIsLocal = false;
let CLodopJsState: 'loading' | 'complete' | undefined;
...
function needCLodop() {
  try {
    ...
    if ((!verTrident) && (!verIE) && (x64)) return true
      ...
      const version = Number(verFF[0].match(/\d+/)?.[0] ?? 0)
      if ((version >= 41) || (x64)) return true
    } else if (verOPR) {
      const version = Number(verOPR[0].match(/\d+/)?.[0] ?? 0)
      if (version >= 32) return true
    } else if ((!verTrident) && (!verIE)) {
      var verChrome = ua.match(/Chrome\D?\d+/i)
      if (verChrome) {
        const version = Number(verChrome[0].match(/\d+/)?.[0] ?? 0)
        if (version >= 41) return true
      }
    }
    return false
  } catch (err) {
...
function getLodopObj(oOBJECT?: LodopClient, oEMBED?: LodopClient): LodopClient | undefined {
  ...
  var LODOP: LodopClient | undefined
  try {
    ...
    if (needCLodop() || isLinuxX86 || isLinuxARM) {
      ...
      if (!LODOP) {
        ...
        if (LODOP.CVERSION < '4.1.5.8') strAlertMessage = strCLodopUpdate
      ...
      if (oOBJECT || oEMBED) {
        ...
        const lodopElement = document.createElement('object')
        lodopElement.setAttribute('width', '0')
        lodopElement.setAttribute('height', '0')
        lodopElement.setAttribute('style', 'position:absolute;left:0px;top:-100px;width:0px;height:0px;')
        if (isWinIE) { lodopElement.setAttribute('classid', 'clsid:2105C259-1E0C-4534-8141-A753534CB4CA') } else { lodopElement.setAttribute('type', 'application/x-print-lodop') }
        document.documentElement.appendChild(lodopElement)
        LODOP = lodopElement as unknown as LodopClient
...
function getLodop(data: A4ExcelPortraitPrintBo, title?: string): LodopClient {
  const LODOP = getLodopObj()
  if (!LODOP) {
    throw new Error('打印控件不可用')
  }
