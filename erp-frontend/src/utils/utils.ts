 ...
 * @type {RegExp}
 */
export const PATTERN_IS_CODE: RegExp = /^.{1,100}$/;
 ...
 * @returns {string}
 */
export const formatDate = function (val: any): string {
  if (!val) return '';
  const m = (val && typeof val.format === 'function')
    ? val
    : typeof val === 'string'
      ? (val.includes(':') ? Moment(val, 'YYYY-MM-DD HH:mm:ss') : Moment(val, 'YYYY-MM-DD'))
      : Moment(val);
 ...
 * @returns {string}
 */
export const formatDateTime = function (val: any, pattern?: string): string {
  if (!val) return '';
  const m = (val && typeof val.format === 'function')
    ? val
    : typeof val === 'string'
      ? (val.includes(':') ? Moment(val, 'YYYY-MM-DD HH:mm:ss') : Moment(val, 'YYYY-MM-DD'))
      : Moment(val);
  return m && m.isValid() ? m.format(pattern || 'YYYY-MM-DD HH:mm:ss') : '';
};
...
export const uuid = function () {
  ...
  s[19] = hexDigits.substr((Number.parseInt(s[19], 16) & 0x3) | 0x8, 1); // bits 6-7 of the clock_seq_hi_and_reserved to 01
 ...
 * @returns {*}
 */
export const swapArrayItem = function <T>(arr: T[], index: number, toIndex: number): T[] {
 ...
 * @param arr
 */
export const combineArr = function <T>(arr: T[][]): T[][] {
  const result = arr.reduce<T[][]>((accArr, currentArr) => {
    const combined: T[][] = [];
    currentArr.forEach((c) => {
      if (accArr.length) {
        accArr.forEach((a) => {
          combined.push(a.concat(c));
        });
      } else {
        combined.push([c]);
      }
