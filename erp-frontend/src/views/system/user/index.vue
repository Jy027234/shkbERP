        ...
        <template #form>
          <j-border>
            <j-form label-width="80px" @collapse="$refs.grid.refreshColumn()">
              ...
              <j-form-item label="单位编码">
                <a-select v-model:value="searchFormData.unitCode" placeholder="全部" allow-clear>
                  <a-select-option
                    v-for="item in $enums.UNIT_CODE.values()"
                    :key="item.code"
                    :value="item.code"
                    >{{ item.desc }}</a-select-option
                  >
...
import { UNIT_CODE } from '@/enums/biz/unitCode';
  import {
    SearchOutlined,
    PlusOutlined,
    ThunderboltOutlined,
  ...
  export default defineComponent({
    ...
    beforeCreate() {
      // 注册枚举
      this.$enums.UNIT_CODE = UNIT_CODE;
    },
    data() {
      return {
        loading: false,
        ...
        tableColumn: [
          ...
          { field: 'telephone', title: '联系电话', width: 120 },
          {
            ...
            width: 100,
            formatter: ({ cellValue }) => {
              return this.$enums.UNIT_CODE.getDesc(cellValue);
            },
          },
          {
            field: 'gender',
