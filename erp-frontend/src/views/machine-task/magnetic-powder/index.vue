    ...
    <page-wrapper content-full-height fixed-height>
      ...
      >
        ...
        <template #status_default="{ row }">
          <a-tag v-if="row.machineTaskStatus === 0" color="orange">待下发</a-tag>
          <a-tag v-else-if="row.machineTaskStatus === 1" color="green">已下发</a-tag>
          <span v-else>-</span>
        </template>
...
<script>
  ...
  export default defineComponent({
    name: 'MachineTaskMagneticPowder',
    components: {
      DownOutlined,
      ReportDialog,
    },
    ...
    methods: {
      ...
      buildSearchFormData() {
        return {
          ...this.searchFormData,
          // 后端参数为 serialNo
          serialNo: this.searchFormData.serialNumber,
        };
      },
      handleCommand({ key }) {
        if (key === 'batchEnable') {
      ...
      createActions(row) {
        return [
          ...(row.machineTaskStatus === 0
            ? [
                {
                  label: '下发任务',
                  onClick: () => {
                    this.$msg.createConfirm('确定下发该任务吗？').then(() => {
                      this.loading = true;
                      api
                        .send(row.taskId)
                        .then(() => {
                          this.$msg.createSuccess('下发成功');
                          this.search();
                        })
                        .catch((err) => {
                          const message = err && (err.code === 'ECONNABORTED' || /timeout|Network Error/i.test(err.message))
                            ? '磁粉机网络连接错误'
                            : (err?.message || '下发任务失败');
                          this.$msg.createError(message);
                        })
                        .finally(() => {
                          this.loading = false;
                        });
