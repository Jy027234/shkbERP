    ...
    data() {
      return {
        ...
        tableColumn: [
          ...
          { field: 'mainProductName', title: '所属组合航材', width: 120 },
    ...
    methods: {
      ...
      delProduct() {
        ...
        for (let i = 0; i < records.length; i++) {
          if (records[i].isFixed) {
            this.$msg.createError('第' + (i + 1) + '行航材是销售订单中的航材，不允许删除！');
            return;
          }
        }
      ...
      validData() {
        ...
        if (
          ...
          this.$msg.createError('销售订单中的航材必须全部或部分出库！');
          return false;
        }
      ...
      // 选择销售订单
      saleOrderChange(e) {
        // 只要选择了销售订单，清空所有航材，然后将销售订单中所有的明细列出来
