<template>
  <div class="simple-app-container">
    <div v-permission="['purchase:order:modify']" v-loading="loading">
        ...
        <template #toolbar_buttons>
          <a-space>
            ...
            <a-button :icon="h(EditOutlined)" @click="batchInputPurchasePrice"
              >批量调整采购价</a-button
            >
            <a-button v-show="false" :icon="h(AlertOutlined)" @click="setGift">设置赠品</a-button>
          </a-space>
        </template>
      ...
      <j-border v-show="false" title="约定支付">
    ...
    methods: {
      ...
      batchInputPurchasePrice() {
        ...
        for (let i = 0; i < records.length; i++) {
          if (records[i].isGift) {
            this.$msg.createError('第' + (i + 1) + '行航材为赠品，不允许录入采购价！');
            return;
          }
        }
      ...
      setGift() {
        const records = this.$refs.grid.getCheckboxRecords();
        if (this.$utils.isEmpty(records)) {
          this.$msg.createError('请选择要设置为赠品的航材数据！');
          return;
        }
      ...
      validData() {
        ...
        for (let i = 0; i < this.tableData.length; i++) {
          ...
          if (this.$utils.isEmpty(product.purchaseNum)) {
            this.$msg.createError('第' + (i + 1) + '行航材采购数量不允许为空！');
            return false;
          }

          if (!this.$utils.isInteger(product.purchaseNum)) {
            this.$msg.createError('第' + (i + 1) + '行航材采购数量必须为整数！');
            return false;
          }

          if (!this.$utils.isIntegerGtZero(product.purchaseNum)) {
            this.$msg.createError('第' + (i + 1) + '行航材采购数量必须大于0！');
            return false;
          }
        }
        ...
        // 如果没有约定支付记录，自动添加一条现金支付
        if (payTypes.length === 0) {
          // 添加默认现金支付
          this.$refs.payType.addPayType({
            payTypeId: '1', // 假设1是现金支付的ID
            payAmount: this.formData.totalAmount,
            text: '系统自动添加'
          });
        } else if (payTypes.length === 1) {
          // 仅有一条约定支付记录，直接更新金额为当前总金额
          ...
          if (!this.$utils.eq(this.formData.totalAmount, totalPayAmount)) {
            // 金额不一致，直接调整为当前总金额
            this.$refs.payType.clear();
            this.$refs.payType.addPayType({
              payTypeId: payTypes[0].payTypeId, // 保持原支付方式ID
              payAmount: this.formData.totalAmount,
              text: payTypes[0].text ? payTypes[0].text : '系统自动调整'
            });
          }
        } else if (payTypes.length > 1) {
          // 如果有多条约定支付记录，验证金额是否正确
          const totalPayAmount = payTypes.reduce(
            (tot, item) => this.$utils.add(tot, item.payAmount),
            0,
          );
