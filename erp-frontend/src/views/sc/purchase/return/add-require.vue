    ...
    methods: {
      ...
      delProduct() {
        ...
        for (let i = 0; i < records.length; i++) {
          if (records[i].isFixed) {
            this.$msg.createError('第' + (i + 1) + '行航材是采购收货单中的航材，不允许删除！');
            return;
          }
        }
      ...
      validData() {
        ...
        for (let i = 0; i < this.tableData.length; i++) {
          ...
          if (!this.$utils.isEmpty(product.returnNum)) {
            if (!this.$utils.isInteger(product.returnNum)) {
              this.$msg.createError('第' + (i + 1) + '行航材退货数量必须为整数！');
              return false;
            }

            if (product.isFixed) {
              if (!this.$utils.isIntegerGeZero(product.returnNum)) {
                this.$msg.createError('第' + (i + 1) + '行航材退货数量不允许小于0！');
                return false;
              }
            } else {
              if (!this.$utils.isIntegerGtZero(product.returnNum)) {
                this.$msg.createError('第' + (i + 1) + '行航材退货数量必须大于0！');
                return false;
              }
            }
        ...
        if (
          ...
          this.$msg.createError('采购收货单中的航材必须全部或部分退货！');
          return false;
        }
      ...
      loadAvailableSerials(row) {
        const receiveSheetDetailId = row.receiveSheetDetailId || (row.isFixed ? row.id : '');
        if (!receiveSheetDetailId || row.serialLoading) {
          return;
        }
        row.serialLoading = true;
        api
          .getAvailableSerials(receiveSheetDetailId)
          .then((serials) => {
      ...
      receiveSheetChange(e) {
        // 只要选择了采购收货单，清空所有航材，然后将采购收货单中所有的明细列出来
