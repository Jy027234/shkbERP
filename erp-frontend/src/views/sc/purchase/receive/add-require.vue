    ...
    <div v-permission="['purchase:receive:add']" v-loading="loading">
      ...
      >
        ...
        <template #toolbar_buttons>
          <a-space>
            ...
            <a-button v-show="false" :icon="h(PlusOutlined)" @click="openBatchAddProductDialog"
              >批量添加航材</a-button
            >
        ...
        <!-- 批次号 列自定义内容 -->
        <template #batchNumber_default="{ row }">
          <a-input v-if="row.isBatch" v-model:value="row.batchNumber" />
          <span v-else>默认批次</span>
        </template>

        <!-- 唯一序列号列表 列自定义内容 -->
        <template #serialNumberList_default="{ row }">
          <a-input v-if="row.isSerial" v-model:value="row.serialNumberList" placeholder="多个序列号用逗号分隔" />
          <span v-else>-</span>
        </template>
        ...
        <!-- 失效日期 列自定义内容 -->
        <template #expiryDate_default="{ row }">
          <a-date-picker 
      ...
      <j-border title="合计">
        <j-form label-width="140px">
          ...
          <j-form-item v-show="false" label="赠品数量" :span="6">
  ...
  export default defineComponent({
    ...
    data() {
      return {
        ...
        tableColumn: [
          ...
          { field: 'taxRate', title: '税率（%）', align: 'right', width: 100, visible: false },
          {
            field: 'batchNumber',
            title: '批次号',
            width: 200,
            align: 'center',
            slots: { default: 'batchNumber_default' },
          },
          {
          ...
          },
          {
            ...
            title: '失效日期',
            width: 120,
            slots: { default: 'expiryDate_default' },
          },
          {
            field: 'description',
            title: '备注',
    ...
    methods: {
      ...
      delProduct() {
        ...
        for (let i = 0; i < records.length; i++) {
          if (records[i].isFixed) {
            this.$msg.createError('第' + (i + 1) + '行航材是采购订单中的航材，不允许删除！');
            return;
          }
        }
      ...
      validData() {
        ...
        for (let i = 0; i < this.tableData.length; i++) {
          ...
          if (!this.$utils.isEmpty(product.receiveNum)) {
            ...
            if (!product.isFixed) {
              this.$msg.createError('第' + (i + 1) + '行航材收货数量不允许为空！');
              return false;
            }
          }
        }
        ...
        if (
          ...
          this.$msg.createError('采购订单中的航材必须全部或部分收货！');
          return false;
        }
          ...
          products: this.tableData
            ...
            .map((t) => {
              const product = {
                ...
                serialNumberList: t.serialNumberList || '',
      ...
      purchaseOrderChange(e) {
        // 只要选择了采购订单，清空所有航材，然后将采购订单中所有的明细列出来
