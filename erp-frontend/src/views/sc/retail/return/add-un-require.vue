    ...
    data() {
      return {
        ...
        tableColumn: [
          ...
          { field: 'isGift', title: '是否赠品', width: 80, slots: { default: 'isGift_default' }, visible: false },
          {
            field: 'discountRate',
            title: '折扣（%）',
            align: 'right',
    ...
    methods: {
      ...
      batchInputTaxPrice() {
        ...
        for (let i = 0; i < records.length; i++) {
          if (records[i].isGift) {
            this.$msg.createError('第' + (i + 1) + '行航材为赠品，不允许录入价格！');
            return;
          }
        }
      ...
      validData() {
        ...
        for (let i = 0; i < this.tableData.length; i++) {
          ...
          if (this.$utils.isEmpty(product.taxPrice)) {
            this.$msg.createError('第' + (i + 1) + '行航材价格不允许为空！');
            return false;
          }

          if (!this.$utils.isFloat(product.taxPrice)) {
            this.$msg.createError('第' + (i + 1) + '行航材价格必须为数字！');
            return false;
          }

          if (product.isGift) {
            if (parseFloat(product.taxPrice) !== 0) {
              this.$msg.createError('第' + (i + 1) + '行航材价格必须等于0！');
              return false;
            }
          } else {
            if (!this.$utils.isFloatGtZero(product.taxPrice)) {
              this.$msg.createError('第' + (i + 1) + '行航材价格必须大于0！');
              return false;
            }
          }

          if (!this.$utils.isNumberPrecision(product.taxPrice, 2)) {
            this.$msg.createError('第' + (i + 1) + '行航材价格最多允许2位小数！');
            return false;
          }
      ...
      // 选择零售出库单
      outSheetChange(e) {
        // 只要选择了零售出库单，清空所有航材，然后将零售出库单中所有的明细列出来
