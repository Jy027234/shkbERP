<template>
  <a-modal
    ...
    :footer="null"
  >
    <div v-if="visible" v-permission="['stock:product-batch']" v-loading="spinning">
      <a-form
        ref="form"
        :label-col="{ span: 4 }"
        :wrapper-col="{ span: 16 }"
      ...
      >
        ...
        <a-form-item label="库存数量">
          <span>{{ getInfo?.quantity }}</span>
        </a-form-item>
        <a-form-item label="批次号">
          <span>{{ getInfo?.batchNumber }}</span>
        </a-form-item>
...
<script>
  ...
  export default defineComponent({
    name: 'UpdateProductStockBatchDialog',
