    ...
    <div v-if="tool">
      ...
      <div class="tool-info">
        <a-descriptions :column="3" size="small" bordered>
          <a-descriptions-item label="管理编号">{{ tool.code }}</a-descriptions-item>
          <a-descriptions-item label="设备名称">{{ tool.name }}</a-descriptions-item>
          <a-descriptions-item label="规格型号">{{ tool.specification }} {{ tool.model }}</a-descriptions-item>
        </a-descriptions>
      </div>
...
import { get as getTool } from '@/api/equipment/tool';
import {
  getToolAttachments,
  uploadToolAttachments,
  deleteToolAttachment,
  batchDeleteToolAttachments
} from '@/api/equipment/tool-attachment';

export default defineComponent({
  name: 'ToolAttachmentManage',
  setup() {
  ...
  methods: {
    ...
    loadToolInfo() {
      ...
      getTool(this.toolId).then(res => {
        this.tool = res;
      }).catch(err => {
        this.createMessage.error('获取工具信息失败');
      });
    },
    ...
    loadAttachments() {
      ...
      this.loading = true;
      getToolAttachments(this.toolId)
        .then(res => {
    ...
    async handleUpload() {
      ...
      try {
        // 调用API上传文件
        await uploadToolAttachments(this.toolId, actualFiles);
    ...
    handleDelete(record) {
      this.loading = true;
      deleteToolAttachment(record.id)
        .then(res => {
