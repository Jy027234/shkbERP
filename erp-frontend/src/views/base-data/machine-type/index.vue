<template>
  <div v-permission="['base-data:machine-type:query']">
      ...
      >
        ...
        <template #toolbar_buttons>
          <a-space>
            ...
            <a-button
              v-permission="['base-data:machine-type:add']"
              type="primary"
              :icon="h(PlusOutlined)"
...
<script>
  ...
  import * as api from '@/api/base-data/machine-type';
    ...
    data() {
      return {
        ...
        tableColumn: [
          ...
          { field: 'name', title: '机型', minWidth: 160, sortable: true },
    ...
    methods: {
      ...
      createActions(row) {
        return [
          ...
          {
            permission: ['base-data:machine-type:modify'],
            label: '修改',
            onClick: () => {
              this.id = row.id;
          ...
          {
            permission: ['base-data:machine-type:delete'],
            label: '删除',
            color: 'error',
            danger: true,
            onClick: () => {
              this.createConfirm({
                title: '提示',
                iconType: 'warning',
                content: '确认删除该机型？删除后将清空关联航材的机型信息。',
                onOk: async () => {
                  try {
