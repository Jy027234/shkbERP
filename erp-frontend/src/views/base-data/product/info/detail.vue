  ...
  >
    <div v-if="visible" v-permission="['base-data:product:info:query']" v-loading="loading">
      <a-descriptions :column="4" bordered>
        <a-descriptions-item label="件号" :span="2">{{ formData.code }}</a-descriptions-item>
        <a-descriptions-item label="名称" :span="2">{{ formData.name }}</a-descriptions-item>
        <a-descriptions-item v-if="false" label="简称" :span="2">{{ formData.shortName }}</a-descriptions-item>
        <a-descriptions-item v-if="false" label="SKU编号" :span="2">{{ formData.skuCode }}</a-descriptions-item>
        <a-descriptions-item v-if="false" label="简码" :span="2">{{
          formData.externalCode
        }}</a-descriptions-item>
        <a-descriptions-item label="分类" :span="2">{{
          formData.categoryName
        }}</a-descriptions-item>
        <a-descriptions-item label="制造商" :span="2">{{ formData.brandName }}</a-descriptions-item>
        <a-descriptions-item label="规格" :span="2">{{ formData.spec }}</a-descriptions-item>
        <a-descriptions-item label="机型" :span="2">{{ formData.machineTypeName || '-' }}</a-descriptions-item>
        <a-descriptions-item label="件号" :span="2">{{ formData.partNumberName || '-' }}</a-descriptions-item>
        ...
        <a-descriptions-item v-if="false" label="进项税率（%）" :span="2">{{
          $enums.PRODUCT_TYPE.NORMAL.equalsCode(formData.productType) ? formData.taxRate : '-'
        }}</a-descriptions-item>
        <a-descriptions-item v-if="false" label="销项税率（%）" :span="2">{{
        ...
        <a-descriptions-item v-if="false" label="销售价（元）" :span="2">{{
          formData.salePrice
        }}</a-descriptions-item>
        <a-descriptions-item v-if="false" label="零售价（元）" :span="2">{{
          formData.retailPrice
        }}</a-descriptions-item>
        <a-descriptions-item v-if="false" label="航材类型" :span="4">{{
          $enums.PRODUCT_TYPE.getDesc(formData.productType)
        }}</a-descriptions-item>
        <a-descriptions-item label="批次号管理" :span="2">
          {{ formData.isBatch ? '是' : '否' }}
        </a-descriptions-item>
        <a-descriptions-item label="序列号管理" :span="2">
          {{ formData.isSerial ? '是' : '否' }}
        </a-descriptions-item>
