...
<script lang="ts">
  import { h, computed, defineComponent, onMounted, onUnmounted, ref, Ref } from 'vue';
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const noticeDetailDialog = ref<InstanceType<typeof NoticeDetail> | null>(null);
      const siteMessageDetailDialog = ref<InstanceType<typeof SiteMessageDetail> | null>(null);
      ...
      const wsTimer: Ref<ReturnType<typeof setInterval> | null> = ref(null);
      ...
      const eventBusOff = ref<Array<() => void>>([]);
      ...
      function onNoticeClick(record: ListItem) {
        ...
        noticeDetailDialog.value?.openDialog();
      }

      function onSiteMessageClick(record: ListItem) {
        siteMessageId.value = record.id;
        siteMessageDetailDialog.value?.openDialog();
      }

      onMounted(() => {
        wsTimer.value = setInterval(wsConnectDaemon, 3000);
      });

      onUnmounted(() => {
        if (wsTimer.value !== null) {
          clearInterval(wsTimer.value);
        }
        disConnectWs();
        eventBusOff.value.forEach((off) => off());
        eventBusOff.value = [];
      });
