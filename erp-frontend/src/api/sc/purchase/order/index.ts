...
import { PurchaseOrderSelectorForReceiveBo } from '@/api/sc/purchase/order/model/purchaseOrderSelectorForReceiveBo';
import { PurchaseOrderSelectorForReceiveVo } from '@/api/sc/purchase/order/model/purchaseOrderSelectorForReceiveVo';
 ...
 * 查询列表（收货选择器）
 */
export function querySelectorForReceive(
  params: PurchaseOrderSelectorForReceiveVo,
): Promise<PageResult<PurchaseOrderSelectorForReceiveBo>> {
  return defHttp.get<PageResult<PurchaseOrderSelectorForReceiveBo>>(
    {
      url: baseUrl + '/query/receive/selector',
      params,
    },
    {
      region,
 ...
 * 修改订单总金额
 */
export function updateTotalAmount(id: string, totalAmount: number): Promise<void> {
  return defHttp.patch<void>(
    {
      url: baseUrl + '/total-amount',
      data: {
        id,
        totalAmount,
      },
