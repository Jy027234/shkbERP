...
public class WebExceptionHandler {
  ...
  public Response handle(UnexpectedTypeException e, HandlerMethod method) {
    ...
    // UnexpectedTypeException means that the server applied an incompatible
    // validation constraint. It is a programming/configuration error rather
    // than bad input supplied by the client.
    BaseException ex = new DefaultSysException();
  ...
  protected void setResponseCode(BaseException e) {
    ...
    ResponseUtil.getResponse().setStatus(resolveHttpStatus(e).value());
  }

  /**
   * Resolve the HTTP transport status without changing the established response-body codes.
   *
   * <p>The starter uses code 500 for both {@link DefaultClientException} and true system
   * failures. The exception hierarchy is therefore needed to distinguish a recoverable business
   * rejection (409) from an unexpected server failure (500).</p>
   */
  static HttpStatus resolveHttpStatus(BaseException e) {

    int code = e.getCode();
    return switch (code) {
      case 400 -> HttpStatus.BAD_REQUEST;
      case 401, 419 -> HttpStatus.UNAUTHORIZED;
      case 403 -> HttpStatus.FORBIDDEN;
      case 410 -> HttpStatus.CONFLICT;
      default -> e instanceof ClientException ? HttpStatus.CONFLICT
