...
public class RetailReturnController extends DefaultBaseController {
  ...
  public void export(@Valid QueryRetailReturnVo vo) {
    ...
    try {
      int pageIndex = 1;
      while (true) {
        PageResult<RetailReturn> pageResult = retailReturnService.query(pageIndex, com.lframework.starter.web.core.controller.ExportSizeSupport.DEFAULT_EXPORT_SIZE,
            vo);
        List<RetailReturn> datas = pageResult.getDatas();
