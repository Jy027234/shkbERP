...
public class SaleReturnController extends DefaultBaseController {
  ...
  public void export(@Valid QuerySaleReturnVo vo) {
    ...
    try {
      int pageIndex = 1;
      while (true) {
        PageResult<SaleReturn> pageResult = saleReturnService.query(pageIndex, com.lframework.starter.web.core.controller.ExportSizeSupport.DEFAULT_EXPORT_SIZE, vo);
        List<SaleReturn> datas = pageResult.getDatas();
