...
public class ReceiveSheetController extends DefaultBaseController {
  ...
  public void export(@Valid QueryReceiveSheetVo vo) {
    ...
    try {
      int pageIndex = 1;
      while (true) {
        PageResult<ReceiveSheet> pageResult = receiveSheetService.query(pageIndex, com.lframework.starter.web.core.controller.ExportSizeSupport.DEFAULT_EXPORT_SIZE,
            vo);
        List<ReceiveSheet> datas = pageResult.getDatas();
