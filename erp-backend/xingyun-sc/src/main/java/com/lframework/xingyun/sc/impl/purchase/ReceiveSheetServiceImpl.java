...
public class ReceiveSheetServiceImpl extends BaseMpServiceImpl<ReceiveSheetMapper, ReceiveSheet>
  ...
  public void approvePass(ApprovePassReceiveSheetVo vo) {
    ...
    List<ReceiveSheetDetail> details = receiveSheetDetailService.list(queryDetailWrapper);
    for (ReceiveSheetDetail detail : details) {
      // 获取商品信息，用于判断是否启用批次和序列号管理
      Product product = productService.findById(detail.getProductId());
      boolean isBatchManage = product.getIsBatch() != null && product.getIsBatch();
      boolean isSerialManage = product.getIsSerial() != null && product.getIsSerial();
      ...
      // 处理批次号
      String batchNumber = isBatchManage ? detail.getBatchNumber() : "DEFAULT";
      
      // 查询是否已存在该批次库存
      LambdaQueryWrapper<ProductStockBatch> queryWrapper =
          com.baomidou.mybatisplus.core.toolkit.Wrappers.lambdaQuery(ProductStockBatch.class)
      ...
      ProductStockBatch existBatch = productStockBatchService.getOne(queryWrapper);
      
      if (existBatch != null) {
          // 批次库存已存在，累加数量
          existBatch.setQuantity(existBatch.getQuantity() + detail.getOrderNum());
          // 更新生产日期和有效期（如果当前为空且新数据不为空）
          if (existBatch.getProductionDate() == null && detail.getProductionDate() != null) {
              existBatch.setProductionDate(detail.getProductionDate());
          }
          if (existBatch.getExpiryDate() == null && detail.getExpiryDate() != null) {
              existBatch.setExpiryDate(detail.getExpiryDate());
          }
          productStockBatchService.updateById(existBatch);
      } else {
          // 批次库存不存在，新增记录
          ProductStockBatch batch = new ProductStockBatch();
          batch.setId(IdUtil.getId());
          batch.setScId(sheet.getScId());
          batch.setProductId(detail.getProductId());
          batch.setQuantity(detail.getOrderNum());
      ...
      // 获取当前处理的批次ID，用于后续关联序列号
      String batchId = existBatch != null ? existBatch.getId() : 
          productStockBatchService.getOne(queryWrapper).getId();
      
      // 处理序列号
      if (isSerialManage && detail.getSerialNumberList() != null && !detail.getSerialNumberList().isEmpty()) {
        String[] serialNumbers = detail.getSerialNumberList().split(",");
        for (String serialNumber : serialNumbers) {
          ...
          serial.setProductId(detail.getProductId());
          serial.setSerialNumber(serialNumber);
          serial.setStockStatus(1); // 在库状态
          serial.setBatchId(batchId);
          serial.setProductionDate(detail.getProductionDate());
          serial.setExpiryDate(detail.getExpiryDate());
  ...
  private void create(ReceiveSheet sheet, CreateReceiveSheetVo vo, boolean receiveRequirePurchase) {
    ...
    for (ReceiveProductVo productVo : vo.getProducts()) {
      ...
      // 设置批次号
      detail.setBatchNumber(StringUtil.isBlank(productVo.getBatchNumber()) ? "DEFAULT" : productVo.getBatchNumber());
      // 设置唯一序列号列表
      detail.setSerialNumberList(StringUtil.isBlank(productVo.getSerialNumberList()) ? StringPool.EMPTY_STR : productVo.getSerialNumberList());
      // 设置生产日期
      if (!StringUtil.isBlank(productVo.getProductionDate())) {
        detail.setProductionDate(LocalDate.parse(productVo.getProductionDate()));
      }
      // 设置失效日期
      if (!StringUtil.isBlank(productVo.getExpiryDate())) {
        detail.setExpiryDate(LocalDate.parse(productVo.getExpiryDate()));
      }
