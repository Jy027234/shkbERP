...
public class SaleOutSheetController extends DefaultBaseController {
  ...
  public void export(@Valid QuerySaleOutSheetVo vo) {
    ...
    try {
      int pageIndex = 1;
      while (true) {
        PageResult<SaleOutSheet> pageResult = saleOutSheetService.query(pageIndex, com.lframework.starter.web.core.controller.ExportSizeSupport.DEFAULT_EXPORT_SIZE,
            vo);
        List<SaleOutSheet> datas = pageResult.getDatas();
