...
public class AuthController extends DefaultBaseController {
  ...
  @ApiOperation("登录")
  @OpenApi
  @OpLog(type = DefaultOpLogType.AUTH, name = "用户登录")
  ...
  @ApiOperation("退出登录")
  @OpenApi
  @OpLog(type = DefaultOpLogType.AUTH, name = "退出登录")
  ...
  static void validateLoginEligibility(AbstractUserDetails user) {
    ...
    if (!user.isCredentialsNonExpired()) {
      throw new UserLoginException("登录凭证已过期，不允许登录！");
    }
