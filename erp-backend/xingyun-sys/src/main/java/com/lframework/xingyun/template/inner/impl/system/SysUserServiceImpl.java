...
public class SysUserServiceImpl extends BaseMpServiceImpl<SysUserMapper, SysUser> implements
  ...
  @OpLog(type = DefaultOpLogType.SYSTEM, name = "修改用户，ID：{}, 编号：{}", params = {"#id",
      "#code"})
  @Transactional(rollbackFor = Exception.class)
  @Override
  ...
  protected SysUser doCreate(CreateSysUserVo vo) {
    ...
    // 设置单位编码
    if (!StringUtil.isBlank(vo.getUnitCode())) {
      record.setUnitCode(vo.getUnitCode());
    }
  ...
  protected void doUpdate(UpdateSysUserVo vo) {
    ...
    LambdaUpdateWrapper<SysUser> updateWrapper = Wrappers.lambdaUpdate(SysUser.class)
        ...
        .set(SysUser::getAvailable, vo.getAvailable())
        .set(SysUser::getDescription,
            StringUtil.isBlank(vo.getDescription()) ? StringPool.EMPTY_STR : vo.getDescription())
        .set(SysUser::getUnitCode, null);
    ...
    // 设置单位编码
    if (!StringUtil.isBlank(vo.getUnitCode())) {
      updateWrapper.set(SysUser::getUnitCode, vo.getUnitCode());
    }
