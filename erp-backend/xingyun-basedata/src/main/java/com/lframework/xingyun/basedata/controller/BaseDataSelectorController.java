...
import com.lframework.xingyun.basedata.bo.machineType.MachineTypeSelectorBo;
...
public class BaseDataSelectorController extends DefaultBaseController {
  ...
  public InvokeResult<PageResult<PartNumberSelectorBo>> selectorPartNumber(@Valid PartNumberSelectorVo vo) {

    PageResult<PartNumberSelectorBo> pageResult = partNumberService.selector(getPageIndex(vo), getPageSize(vo),
        vo);
   ...
   */
  @ApiOperation("加载件号")
  @PostMapping("/partnumber/load")
  public InvokeResult<List<PartNumberSelectorBo>> loadPartNumber(
      @RequestBody(required = false) List<String> ids) {
    ...
    List<PartNumber> datas = ids.stream().filter(StringUtil::isNotBlank)
        .map(t -> partNumberService.findById(t))
        .filter(Objects::nonNull).collect(Collectors.toList());

    List<PartNumberSelectorBo> results = datas.stream().map(t -> {
      PartNumberSelectorBo bo = new PartNumberSelectorBo();
      bo.setId(t.getId());
      bo.setCode(t.getCode());
      bo.setName(t.getName());
      bo.setMachineTypeId(t.getMachineTypeId());
      return bo;
  ...
  public InvokeResult<PageResult<MachineTypeSelectorBo>> selector(@Valid MachineTypeSelectorVo vo) {

    PageResult<MachineType> pageResult = machineTypeService.selector(getPageIndex(vo), getPageSize(vo),
        vo);

    List<MachineType> datas = pageResult.getDatas();
    List<MachineTypeSelectorBo> results = null;

    if (!CollectionUtil.isEmpty(datas)) {
      results = datas.stream().map(MachineTypeSelectorBo::new).collect(Collectors.toList());
    }
   ...
   */
  @ApiOperation("加载机型")
  @PostMapping("/machinetype/load")
  public InvokeResult<List<MachineTypeSelectorBo>> loadMachineType(
      @RequestBody(required = false) List<String> ids) {
    ...
    List<MachineType> datas = ids.stream().filter(StringUtil::isNotBlank)
        .map(t -> machineTypeService.findById(t))
        .filter(Objects::nonNull).collect(Collectors.toList());
    List<MachineTypeSelectorBo> results = datas.stream().map(MachineTypeSelectorBo::new)
        .collect(
            Collectors.toList());
    return InvokeResultBuilder.success(results);
  }
  ...
  public InvokeResult<PageResult<RepairTypeSelectorBo>> selectorRepairType(@Valid RepairTypeSelectorVo vo) {

    PageResult<RepairType> pageResult = repairTypeService.selector(getPageIndex(vo), getPageSize(vo),
        vo);
   ...
   */
  @ApiOperation("加载维修类型")
  @PostMapping("/repairtype/load")
  public InvokeResult<List<RepairTypeSelectorBo>> loadRepairType(
      @RequestBody(required = false) List<String> ids) {
    ...
    List<RepairType> datas = ids.stream().filter(StringUtil::isNotBlank)
        .map(t -> repairTypeService.findById(t))
        .filter(Objects::nonNull).collect(Collectors.toList());
    List<RepairTypeSelectorBo> results = datas.stream().map(RepairTypeSelectorBo::new)
        .collect(
            Collectors.toList());
    return InvokeResultBuilder.success(results);
  }
