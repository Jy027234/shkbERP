  ...
  protected void afterAllAnalysed(AnalysisContext context) {
    ...
    for (int i = 0; i < datas.size(); i++) {
      ...
      if (StringUtil.isNotBlank(data.getIsBatch())) {
        if ("是".equals(data.getIsBatch()) || "true".equalsIgnoreCase(data.getIsBatch()) || "1".equals(data.getIsBatch())) {
          record.setIsBatch(Boolean.TRUE);
        } else {
          record.setIsBatch(Boolean.FALSE);
        }
      } else {
        record.setIsBatch(Boolean.FALSE);
      }
      ...
      if (StringUtil.isNotBlank(data.getIsSerial())) {
        if ("是".equals(data.getIsSerial()) || "true".equalsIgnoreCase(data.getIsSerial()) || "1".equals(data.getIsSerial())) {
          record.setIsSerial(Boolean.TRUE);
        } else {
          record.setIsSerial(Boolean.FALSE);
        }
      } else {
        record.setIsSerial(Boolean.FALSE);
      }
