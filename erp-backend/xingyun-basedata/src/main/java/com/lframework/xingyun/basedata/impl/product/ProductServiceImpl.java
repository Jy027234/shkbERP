...
public class ProductServiceImpl extends BaseMpServiceImpl<ProductMapper, Product> implements
  ...
  public String create(CreateProductVo vo) {
    ...
    // 设置机型
    if (StringUtil.isNotBlank(vo.getMachineTypeId())) {
      data.setMachineTypeId(vo.getMachineTypeId());
    } else {
      data.setMachineTypeId(null);
    }
    
    // 设置批次号管理和序列号管理状态
    data.setIsBatch(vo.getIsBatch());
    data.setIsSerial(vo.getIsSerial());
    ...
    createProductPurchaseVo.setPrice(purchasePrice);
  ...
  public void update(UpdateProductVo vo) {
    ...
    LambdaUpdateWrapper<Product> updateWrapper = Wrappers.lambdaUpdate(Product.class)
        ...
        .set(Product::getMachineTypeId, 
            StringUtil.isBlank(vo.getMachineTypeId()) ? null : vo.getMachineTypeId())
        // 设置批次号管理和序列号管理状态
        .set(Product::getIsBatch, vo.getIsBatch())
        .set(Product::getIsSerial, vo.getIsSerial())
